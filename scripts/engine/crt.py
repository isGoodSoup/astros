...
class CRT:
    def __init__(self, screen, style=1, virtual_resolution=(800, 600),
                 cpu_only=False,
                 fullscreen=False):
        pygame.init()
        ...
        if not self.cpu_only:
            ...
            self.prog = self.ctx.program(
                vertex_shader=open(
                resource_path('assets/shaders/VERTEX_SHADER.glsl')).read(),
                fragment_shader=open(resource_path(
                'assets/shaders/FRAGMENT_SHADER.glsl')).read())
    ...
    def change_shader(self):
        if not self.cpu_only:
            self.__init__(self.screen, (self.style + 1) % 3,
                          self.virtual_resolution)

    def render(self, surface):
        if not self.cpu_only:
            surface = pygame.transform.scale(surface, self.virtual_resolution)
            surface = surface.convert_alpha()
            texture_data = pygame.image.tostring(surface, "RGBA", False)
            self.screen_texture.write(texture_data, alignment=1)
    ...
    def fullscreen(self, real_resolution):
        if not self.cpu_only:
            if not self.fullscreen:
                pygame.display.set_mode(real_resolution,
