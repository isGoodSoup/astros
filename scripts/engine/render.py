...
from scripts.system.lang import LANGS
from scripts.engine.runtime import get_desc
...
from scripts.engine.utils import wrap_text
...
def render_skills_tab(game, screen, rect, game_font):
    title_text, perks = game.local.t('game.victory'), game.local.t("game.hud.perks",
    ...
    for i, (skill, pos) in enumerate(zip(game.state.current_phase_options,
        ...
        if skill.hovered and skill.description:
            ...
            text = game.local.t(skill.description_key)
...
def render_stats_tab(game, screen, rect, game_font):
    ...
    stats = [
        game.local.t("game.hud.hitpoints", hp=int(game.ship.hitpoints),
                max_hp=int(game.ship.max_hitpoints)),
        game.local.t("game.hud.shield", shield=int(game.ship.shield),
                max_shield=int(game.ship.max_shield)),
        game.local.t("game.hud.ammo", ammo=int(available_ammo),
                max_ammo=int(game.ship.arsenal)),
        game.local.t("game.hud.level", level=int(game.ship.level)),
        game.local.t("game.hud.xp", xp=int(game.ship.xp),
                xp_to_next_level=int(game.ship.xp_to_next_level)),
        game.local.t("game.hud.crit_chance",
                crit_chance=int(game.ship.crit_chance * 100)),
        game.local.t("game.hud.crit_multiplier",
...
def render_settings_tab(game, screen, rect, game_font):
    header_surface = game_font.render(game.local.t('game.settings.header'), True,
                                      COLOR_WHITE)
    ...
    for i, setting in enumerate(SETTINGS_DEFINITION):
        label = game.local.t(setting["label"])
